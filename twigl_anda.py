...
#    powers=[[1e+06]]    
    ...
    for t in np.arange(0,tfinal+2*dt,dt):
        if EQUILIBRIUM:
            C_t_1 = C0.copy()
            C_t = C0.copy()
        else:
        ...
        OsArgs = (FILENAME.replace('.cii','S.cii'),_NOG,Nx,Ny,Nz,EQUILIBRIUM)
...
#        powers.append([float(pi.groups()[0]) for pi in p.finditer(fid.read())])
    ...
    Normalized_Pow = np.array(powers)/powers[0]
    ...
    plt.plot(np.arange(0,tfinal+2*dt,dt),Normalized_Pow,DATA[:,0],DATA[:,1],'ro',DATA[:,0],DATA[:,2],'go')